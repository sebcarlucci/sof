...
static int crossover_init_coef(struct comp_data *cd, int nch)
{
	...
	for (i = 0; i < nch; i++) {
		err = crossover_init_coef_ch(crossover, &cd->state[i],
					     config->num_sinks);
 ...
 *
 * The function can only be called after the buffers have been initilized.
 */
				  ...
				  struct sof_ipc_ctrl_data *cdata)
{
	...
	case SOF_CTRL_CMD_BINARY:
		...
		if (crossover_validate_config(dev, request) < 0) {
			/* If config is invalid then just don't set it.
			 * Return 0 so that the firmware still gets
			 * initialized
			 */
...
static int crossover_copy(struct comp_dev *dev)
{
	 ...
	 * sinks[0...num_assigned_sinks] should all be assigned.
	 */
	num_assigned_sinks = crossover_assign_sinks(dev, cd->config, sinks);
	if (cd->config && num_assigned_sinks != cd->config->num_sinks) {
		comp_err(dev, "crossover_copy(), number of assigned sinks (%i) does not match number of sinks in config (%i).",
	...
	for (i = 0; i < num_assigned_sinks; i++) {
	...
	cd->crossover_process(dev, source, sinks, num_assigned_sinks, frames);
