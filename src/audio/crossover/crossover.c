				...
				uint32_t pipe_id)
{
	...
	comp_cl_err(&comp_crossover, "get_stream_index() error: couldn't find any assignment for connected pipeline %u",
		    pipe_id);
				  ...
				  struct comp_buffer **sinks)
{
	...
	list_for_item(comp_list, &dev->bsink_list) {
		sink = container_of(comp_list, struct comp_buffer, source_list);
...
static int crossover_init_coef(struct comp_data *cd, int nch)
{
	...
	if (num_sinks > SOF_CROSSOVER_MAX_STREAMS || num_sinks == 0) {
		comp_cl_err(&comp_crossover, "crossover_init_coef(), invalid number of sinks set: num_sinks=%i",
			    num_sinks);
		return -EINVAL;
	}
				     ...
				     struct sof_crossover_config *config)
{
	...
	if (config->num_sinks > SOF_CROSSOVER_MAX_STREAMS || !config->num_sinks) {
		comp_err(dev, "crossover_validate_config(), invalid num_sinks %i, expected number between 1 and %i",
			    ...
			    struct sof_ipc_stream_params *params)
{
	...
	if (cd->config && crossover_validate_config(dev, cd->config) < 0) {
		comp_err(dev, "crossover_params(), invalid binary config format");
		return -EINVAL;
	}
	return 0;
}
				  ...
				  struct sof_ipc_ctrl_data *cdata)
{
	...
	case SOF_CTRL_CMD_BINARY:
		...
		if (!crossover_validate_config(dev, request)) {
			comp_err(dev, "crossover_cmd_set_data(), invalid binary config format");
			return -EINVAL;
		}
...
static int crossover_copy(struct comp_dev *dev)
{
	...
	for (i = 0; i < SOF_CROSSOVER_MAX_STREAMS; i++) {
		if (!sinks[i])
			continue;
		buffer_lock(sinks[i], &flags);
		avail = audio_stream_avail_frames(&source->stream,
						  &sinks[i]->stream);
		frames = MIN(frames, avail); /* TODO does this work with an XRUN */
...
static int crossover_prepare(struct comp_dev *dev)
{
	...
	list_for_item(sink_list, &dev->bsink_list) {
		sink = container_of(sink_list, struct comp_buffer, source_list);
		/* Validate that the formats are the same */
		if (cd->source_format != sink->stream.frame_fmt) {
			comp_err(dev, "src_prepare(): Source fmt %d and sink fmt %d are different for sink %d.",
